...
export async function fetchDeps(
  ...
  // Always load at least the root issue's immediate relations (depth 1).
  const effectiveMaxDepth = Math.max(maxDepth, 1);
  await fetchDepsRecursive(host, issue.id, 1, effectiveMaxDepth, relations, followDirs, settings, issues);
