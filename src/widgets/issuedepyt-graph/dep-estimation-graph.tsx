...
import { durationToDays, isPastDate, getDuration, calcBusinessDays } from "./time-utils";
...
const getIssueWork = (
  issue: IssueInfo
): { estimatedDays: number; scheduledDays: number } | null => {
  ...
  return { estimatedDays, scheduledDays };
};
  ...
  useEffect(() => {
    if (timeline.current && items.current && groups.current) {
      ...
      const timelineItems: Array<Graph2dItem> = visibleIssues.flatMap((issue) => {
        const { estimatedDays, scheduledDays } = getIssueWork(issue) || {
          estimatedDays: 0,
          scheduledDays: 0,
        };
        if (estimatedDays === 0 || scheduledDays === 0) {
          return [];
        }
        const startDate = issue.startDate as Date;
        const dueDate = issue.dueDate as Date;
      ...
      const timelineGroups: Array<Graph2dGroup> = visibleIssues.map((issue, idx) => {
        ...
        const work = estimatedDays / scheduledDays;
        return {
          id: issue.id,
          content: `${issue.idReadable}: ${issue.summary} (${work.toFixed(2)})`,
